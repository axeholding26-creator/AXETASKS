...
const JWT_SECRET = process.env.JWT_SECRET || 'axetask-secure-secret-key-2026';
...
async function startServer() {
  ...
  app.get('/api/health', (req, res) => {
    res.json({ status: 'ok', database: 'cloud_sql_postgres', time: new Date().toISOString() });
  });

  // Auth: Demo Users for quick test switching
  ...
  // Users List (for assignment & inviting & admin management)
  ...
  app.post('/api/users', authMiddleware, async (req: AuthenticatedRequest, res) => {
    ...
    try {
      ...
      const newUser = await db.createUserWithRole(email, defaultPassword, name, role || 'member');
